...
void stuffingframe(unsigned char *p_Message)
{
	unsigned int cnt=0;
	unsigned int ones=0;
	unsigned char *P_out=0x0;
	unsigned int flag_stuffing=0;
	int bitcnt=0;
	while(cnt++<strlen((char*)p_Message))
	{
		for(bitcnt=0;bitcnt<6;bitcnt++)
		{

			if(*p_Message&(1<<bitcnt))//Test a un
			{
				ones++;
...
void makeax25payload(const unsigned char*stationSSID,const unsigned char*SSIDDest,const unsigned char *Pmessage,P_AX_25 *ax25_frame){
	...
	(*ax25_frame)->send_id=malloc(sizeof(unsigned char*)*strlen((char*)stationSSID));
	...
	strcpy((char*)(*ax25_frame)->endmessage,"~");
	strcat((char*)(*ax25_frame)->endmessage,(const char*)(unsigned char*)(*ax25_frame)->fullmessage);
